...
module.exports = {
	async execute(client, db, config, logger) {
		...
		if (data.is_live) { // Checks if broadcaster is live
			...
			let LiveTime = await database.get(db, 'SELECT "value" FROM "' + process.env.twitchNotificationsGuild + '" WHERE key="LiveTime"')
			if (LiveTime == data.started_at) { // Checks if we have already notified for this live
				let LiveTitle = await database.get(db, 'SELECT "value" FROM "' + process.env.twitchNotificationsGuild + '" WHERE key="LiveTitle"')
				if (!LiveTitle) {
					database.updateKey(db, process.env.twitchNotificationsGuild, 'LiveTitle', sha1(data.title))
				}
				...
				if (sha1(data.title) == LiveTitle) { // If the title in the message and title of stream is the same, do nothing
					...
					database.updateKey(db, process.env.twitchNotificationsGuild, 'LiveTitle', sha1(data.title)) // Put the new title in the db
					let MessageId = await database.get(db, 'SELECT "value" FROM "' + process.env.twitchNotificationsGuild + '" WHERE key="LiveMessageId"') // Get the message id of the notiication we sent
					if (MessageId) {
				...
				database.updateKey(db, process.env.twitchNotificationsGuild, 'LiveTime', data.started_at) // Put the time of live in db so we don't notify twice
				const sentMessage = await notificationChannel.send(notificationMessageContent) // Notify for the live in the right channel
				database.updateKey(db, process.env.twitchNotificationsGuild, 'LiveMessageId', sentMessage.id) // Put the notification message id in db so we can edit the message later
			}
		}
	}
}
