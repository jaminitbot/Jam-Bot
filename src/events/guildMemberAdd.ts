...
export async function sendUserToModlog(client: BotClient, member: GuildMember) {
	...
	embed.setDescription(i18next.t('events:guildMemberAdd.EMBED_DESCRIPTION', { userId: member.id, creationDate: dayjs().to(member.user.createdTimestamp), numberOfUsers: member.guild.memberCount }))
