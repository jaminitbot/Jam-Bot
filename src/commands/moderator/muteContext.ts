...
export async function executeContextMenu(client: BotClient, interaction: ContextMenuInteraction) {
    ...
    const targetUser = await interaction.guild.members.fetch(interaction.targetId)
    ...
    const muteResult = await mute(interaction.guild, targetUser.id, formattedReason, parsedDuration)
    if (muteResult == 0) {
        interaction.reply({ content: i18next.t('mod.ACTION_SUCCESSFUL', { tag: targetUser.user.tag, action: 'muted', reason: reason ?? i18next.t('mod.NO_REASON_SPECIFIED') }), allowedMentions: { parse: [] } })
