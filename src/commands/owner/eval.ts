...
async function runEvalCommand(commandToRun, logger, message) {
	...
	commandOutput = String(commandOutput).replace(
		secretsRegex,
		i18next.t("eval.SECRET")
	);
	commandOutput = String(commandOutput);
	if (commandToRun.length > 1024) {
		const uploadedHasteLocation = await uploadToHasteBin(
			logger,
			commandToRun
		);
		if (uploadedHasteLocation) {
			embed.addField(
				i18next.t("eval.COMMAND"),
				i18next.t("eval.TOO_LARGE_UPLOADED", {
					context: "SUCCESS",
					url: uploadedHasteLocation,
					type: "command",
				})
			);
		} else {
...
export async function execute(
	...
	if (!args[0])
		return message.reply(i18next.t("eval.NO_ARGUMENTS_SPECIFIED"));
	const command = args.splice(0).join(" ");
	const sentMessage = await message.channel.send({
		content: i18next.t("eval.COMMAND_RUNNING"),
	});
