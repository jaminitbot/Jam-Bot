...
export async function execute(client: BotClient, message: Message, args) {
	...
	if (result == 0) {
		...
		message.channel.send('Whoops, suggestions are disabled in this server!')
	}
}
export async function executeSlash(client: BotClient, interaction: CommandInteraction) {
	...
	if (result == 0) {
		...
		interaction.reply({ content: 'Whoops, suggestions are disabled in this server!', ephemeral: true })
	}
}