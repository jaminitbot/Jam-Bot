...
async function returnHelpEmbed(client: BotClient, commandToGet: string, prefix: string, userId: string) {
    ...
    if (commandToGet) {
        ...
        if (command && !(command.permissions && command.permissions.includes('OWNER') && !isBotOwner(userId)) && (typeof command.execute == 'function' || typeof command.executeSlash == 'function')) {
