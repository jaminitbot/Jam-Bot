...
function returnDebugEmbed(sentMessageTimestamp: number, newMessageTimestamp: number, clientUptime: number, websocketPing: number, guildId: string) {
	const embed = new MessageEmbed
	embed.setTitle(i18next.t('debug.DEBUG_INFORMATION'))
	embed.addField(i18next.t('debug.ROUNDTRIP'), `${sentMessageTimestamp - newMessageTimestamp}ms`, true)
	embed.addField(i18next.t('debug.API_LATENCY'), `${websocketPing}ms`, true)
	const uptimeDate = dayjs(Date.now() - clientUptime).format("HH:mm:ss [-] DD/MM/YYYY")
...
export async function execute(client: BotClient, message: Message, args: Array<unknown>) {
	...
	const embed = returnDebugEmbed(sentMessage.createdTimestamp, message.createdTimestamp, client.uptime, client.ws.ping, message.guild.id)
