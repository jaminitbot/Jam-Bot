...
module.exports = {
	...
	async execute(client, message, args, db, logger) {
		...
		let type
		if (args[args.length -1] == 'gif') { // Gif commands also uses google image search, update wording accordingly
			type = 'gif'
		} else {
			type = 'image'
		}
		...
		message.channel.send(`:mag_right: Finding ${type}...`).then(sent => {
			...
			gis(searchOptions, function (error, results) {
				if (error) return sent.edit('An error occured while getting your results :c')
